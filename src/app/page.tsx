...
import { ListTodo, MoveRight, Boxes, DollarSign, Users } from "lucide-react";

export default function HomePage() {
  const tasks = useStore((state) => state.tasks);
  const assets = useStore((state) => state.assets);
  const users = useStore((state) => state.users);
  const totalAssetValue = assets.reduce((sum, a) => sum + a.price * a.quantity, 0);
    ...
    <main className="flex-1 p-4 grid grid-cols-2 md:grid-cols-3 sm:grid-cols-4 gap-4">
      <section className="bg-[#00A15D] p-4 rounded-lg shadow-xl flex items-center justify-between">
        <div className="flex-shrink-0">
          <ListTodo
            className="2xl:w-20 2xl:h-20 xl:w-16 xl:h-16 lg:w-12 lg:h-12 sm:w-10 sm:h-10 sm:block hidden"
            color="#FFFFFF"
          />
        </div>
        ...
        <div className="flex-shrink-0">
          ...
          >
            <MoveRight
              className="2xl:w-10 2xl:h-10 xl:w-8 xl:h-8 w-4 h-4"
              color="#FFFFFF"
            />
          </a>
        </div>
      ...
      <section className="bg-blue-600 p-4 rounded-lg shadow-xl flex items-center justify-between">
        ...
        <div className="text-left">
          <h2 className="2xl:text-2xl xl:text-xl lg:text-lg sm:text-sm text-[8px] font-bold font-sans text-white">
            Asset price: RM{totalAssetValue.toLocaleString()}
          </h2>
        </div>
        <div className="flex-shrink-0">
          <a
